...
def getPid(db_list):
    pidList = []
    global conn, cur
    cur.execute('SELECT pid FROM {} WHERE status<>"todo"'.format(db_list))
    rows = cur.fetchall()
    for row in rows:
        pidList.append(row[0])
    print('共计:', len(pidList), '件商品待获取')
    return pidList

def getItem(db, pid):
    surl = 'https://sf.taobao.com/sf_item/'
    url = surl + '{}'.format(pid) + '.htm'
    # print(url)
    try:
        soup = getSoupObject(url)
        nurl = soup('div', {'id':'J_NoticeDetail'})[0]['data-from']
        nurl = 'https:' + nurl
        soupn = getSoupObject(nurl)
        js = json.loads(data)
